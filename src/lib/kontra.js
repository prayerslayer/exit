...
(function() {
  ...
  function circleRectCollision(object) {
    let dx =
      pointer.x -
      Math.max(object.x, Math.min(pointer.x, object.x + object.width));
    let dy =
      pointer.y -
      Math.max(object.y, Math.min(pointer.y, object.y + object.height));
...
(function() {
  ...
  kontra.pool = function(properties) {
    ...
    if (
      !properties.create ||
      (!(obj = properties.create()) || !(obj.update && obj.init && obj.isAlive))
    ) {
      throw Error(
    ...
    return {
      ...
      _g(object) {
        ...
        let intersectsTopQuadrants =
          ...
          object.y + object.height >= this.bounds.y;
        let intersectsBottomQuadrants =
          object.y + object.height >= horizontalMidpoint &&
          object.y < this.bounds.y + this.bounds.height;

        // object intersects with the left quadrants
        if (
          object.x < verticalMidpoint &&
          object.x + object.width >= this.bounds.x
        ) {
        ...
        if (
          ...
          object.x < this.bounds.x + this.bounds.width
        ) {
          // top right
          if (intersectsTopQuadrants) {
            indices.push(1);
    ...
    let tileEngine = {
        ...
        [].concat(tilesets).map(function(tileset) {
          ...
          if (!firstGrid) {
            ...
            if (tileEngine.tilesets.length > 0) {
              ...
              tiles =
                ((lastTileset.image.width / tileWidth) | 0) *
      ...
      renderLayer: function renderLayer(name) {
        ...
        let viewHeight = Math.min(
          Math.ceil(canvasHeight / tileHeight) + 1,
          height
        );
